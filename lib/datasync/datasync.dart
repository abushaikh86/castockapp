...
class _DataSyncState extends State<DataSync> {
  ...
  Future<String> getSyncDate() async {
    String syncDate = await dbHelper!.getLastSyncDate();
    return syncDate;
  }
              ...
              child: FutureBuilder<String>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    return Text(
                      "Last Sync Date: Loading...",
                      style: TextStyle(color: Colors.black),
                    );
                  } else if (snapshot.hasError) {
                    return Text(
                      "Error fetching sync date",
                      style: TextStyle(color: Colors.red),
                    );
                  } else {
                    return Text(
                      "Last Sync Date: ${snapshot.data}",
                      style: TextStyle(color: Colors.black),
                    );
                  }
                },
              ...
              children: [
                Flexible(
                  child: ElevatedButton(
                      ...
                      child: Text('Import From Server',
                          style: TextStyle(color: Colors.white, fontSize: 15)),
                      style: ElevatedButton.styleFrom(
                        primary:
                            constants.mainColor, //background color of button
                ...
                Flexible(
                  child: ElevatedButton(
                      onPressed: () async {
                        ...
                        final response =
                            await http.post(Uri.parse(url), body: dataPayload);
                        print(response.body);
                      },
