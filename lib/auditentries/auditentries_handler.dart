...
class AuditentriesDBHelper {
  ...
  Future<List<AuditEntriesModel>> getAuditEntriesList(
      companyId, auditId) async {
    var dbClient = await db;
    final List<Map<String, Object?>> queryResult = await dbClient!.rawQuery(
        "Select * from audit_wise_entries where company_id='$companyId' AND audit_id='$auditId'");
    // print(queryResult);
  ...
  Future<int> delete(int entryId) async {
    var dbClient = await db;
    return await dbClient!.delete('audit_wise_entries',
  ...
  Future<int> update(AuditEntriesModel auditEntriesModel) async {
    ...
    return await dbClient!.update(
        'audit_wise_entries', auditEntriesModel.toMap(),
        where: 'entry_id=?', whereArgs: [auditEntriesModel.entryId]);
  }
}
