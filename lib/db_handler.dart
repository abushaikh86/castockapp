...
class DBHelper {
  ...
  _onCreate(Database db, int version) async {
    await db.execute(
        "CREATE TABLE IF NOT EXISTS admin_users (admin_user_id INTEGER , first_name TEXT NULL, last_name TEXT NULL, email TEXT NULL, password TEXT NULL, mobile_no TEXT NULL)");
    await db.execute(
        "CREATE TABLE IF NOT EXISTS company (company_id INTEGER , company_name TEXT NULL, company_address TEXT NULL, cm_mobile TEXT NULL, city TEXT NULL)");
    await db.execute(
        "CREATE TABLE IF NOT EXISTS brand (brand_id INTEGER , brand_name TEXT NULL, company_id TEXT NULL)");
    await db.execute(
        "CREATE TABLE IF NOT EXISTS format (format_id INTEGER , format_name TEXT NULL, brand_id TEXT NULL)");
    await db.execute(
        "CREATE TABLE IF NOT EXISTS variant (variant_id INTEGER , brand_id TEXT NULL, variant_name TEXT NULL, format_id TEXT NULL)");
    await db.execute(
        "CREATE TABLE IF NOT EXISTS warehouse (warehouse_id INTEGER , warehouse_name TEXT NULL, company_id TEXT NULL)");
    await db.execute(
        "CREATE TABLE IF NOT EXISTS product (product_id INTEGER , product_name TEXT NULL, item_number TEXT NULL, company_id TEXT NULL, format_id TEXT NULL, variant_id TEXT NULL, brand_id TEXT NULL, warehouse_id TEXT NULL, system_unit TEXT NULL, valuation_per_unit TEXT NULL, weight TEXT NULL, mrp TEXT NULL, combi_type TEXT NULL, pcs_cases TEXT NULL, total_stock_value TEXT NULL, mfg_date TEXT NULL, mfg_month TEXT NULL, mfg_year TEXT NULL, exp_date TEXT NULL, exp_month TEXT NULL, exp_year TEXT NULL)");
    await db.execute(
        "CREATE TABLE IF NOT EXISTS audit (audit_id INTEGER , company_id TEXT NULL, audit_description TEXT NULL, audit_status TEXT NULL)");
    await db.execute(
        "CREATE TABLE IF NOT EXISTS audit_wise_entries (entry_id INTEGER , audit_id TEXT NULL, company_id TEXT NULL, product_id TEXT NULL, brand_id TEXT NULL, format_id TEXT NULL, variant_id TEXT NULL, warehouse_id TEXT NULL, user_id TEXT NULL, admin_user_name TEXT NULL, mfg_month TEXT NULL, mfg_year TEXT NULL, exp_month TEXT NULL, exp_year TEXT NULL, actual_unit TEXT NULL, system_unit TEXT NULL, weight TEXT NULL, mrp TEXT NULL,  valuation_per_unit TEXT NULL, total_amount TEXT NULL, factor_1 TEXT NULL,  oprator TEXT NULL, factor_2 TEXT NULL, calculation_arr TEXT NULL, city TEXT NULL, product_name TEXT NULL, company_name TEXT NULL, brand_name TEXT NULL, format_name TEXT NULL, variant_name TEXT NULL, warehouse_name TEXT NULL, combi_type TEXT NULL, pcs_cases TEXT NULL, total_stock_value TEXT NULL, mfg_date TEXT NULL, exp_date TEXT NULL, created_at TEXT NULL, updated_at TEXT NULL, deleted_at TEXT NULL)");
    await db.execute(
        "CREATE TABLE IF NOT EXISTS sync_date (sync_id INTEGER , sync_code TEXT NULL, sync_date TEXT NULL)");
  }
  ...
  Future<int> delete(int auditId) async {
    ...
    return await dbClient!
        .delete('audit', where: 'audit_id=?', whereArgs: [auditId]);
  }

  Future<int> update(AuditModel auditModel) async {
    var dbClient = await db;
    return await dbClient!.update('audit', auditModel.toMap(),
        where: 'audit_id=?', whereArgs: [auditModel.auditId]);
  }
  ...
  Future<List<GetAuditEntriesData>> getAuditEntriesListArray() async {
    ...
    List<GetAuditEntriesData> list = res.isNotEmpty
        ? res.map((c) => GetAuditEntriesData.fromJson(c)).toList()
        : [];
    return list;
  }
  ...
  Future<List<GetBrandData>> getBrandListByCompany(var companyId) async {
    ...
    final res = await dbClient!
        .rawQuery("SELECT * FROM brand where company_id='$companyId'");
    List<GetBrandData> list =
        res.isNotEmpty ? res.map((c) => GetBrandData.fromJson(c)).toList() : [];
    return list;
  }
  ...
  Future<int> deleteBrand(int brandId) async {
    var dbClient = await db;
    return await dbClient!
        .delete('brand', where: 'brand_id=?', whereArgs: [brandId]);
  }

  Future<int> updateBrand(BrandModel brandModel) async {
    var dbClient = await db;
    return await dbClient!.update('brand', brandModel.toMap(),
        where: 'brand_id=?', whereArgs: [brandModel.brandId]);
  }
  ...
  Future<List<GetFormatData>> getFormatListArray() async {
    ...
    List<GetFormatData> list = res.isNotEmpty
        ? res.map((c) => GetFormatData.fromJson(c)).toList()
        : [];
    return list;
  }
  ...
  Future<List<GetFormatData>> getFormatListByBrand(var brandId) async {
    ...
    final res = await dbClient!
        .rawQuery("SELECT * FROM format where brand_id='$brandId'");
    List<GetFormatData> list = res.isNotEmpty
        ? res.map((c) => GetFormatData.fromJson(c)).toList()
        : [];
    return list;
  ...
  Future<int> deleteFormat(int formatId) async {
    var dbClient = await db;
    return await dbClient!
        .delete('format', where: 'format_id=?', whereArgs: [formatId]);
  }

  Future<int> updateFormat(FormatModel formatModel) async {
    var dbClient = await db;
    return await dbClient!.update('format', formatModel.toMap(),
        where: 'format_id=?', whereArgs: [formatModel.formatId]);
  }
  ...
  Future<List<GetVariantData>> getVariantListArray() async {
    ...
    List<GetVariantData> list = res.isNotEmpty
        ? res.map((c) => GetVariantData.fromJson(c)).toList()
        : [];
    return list;
  }

  Future<List<GetVariantData>> getVariantListByBrandAndFormat(
      var brandId, var formatId) async {
    var dbClient = await db;
    // print("SELECT * FROM format where brand_id='$brandId'");
    final res = await dbClient!.rawQuery(
        "SELECT * FROM variant where brand_id='$brandId' and format_id='$formatId'");
    List<GetVariantData> list = res.isNotEmpty
        ? res.map((c) => GetVariantData.fromJson(c)).toList()
        : [];
    return list;
  ...
  Future<int> deleteVariant(int variantId) async {
    var dbClient = await db;
    return await dbClient!
        .delete('variant', where: 'variant_id=?', whereArgs: [variantId]);
  }

  Future<int> updateVariant(VariantModel variantModel) async {
    var dbClient = await db;
    return await dbClient!.update('variant', variantModel.toMap(),
        where: 'variant_id=?', whereArgs: [variantModel.variantId]);
  }
  ...
  Future<List<WarehouseModel>> getWarehouseList() async {
    var dbClient = await db;
    final List<Map<String, Object?>> queryResult =
        await dbClient!.query("warehouse");
  ...
  Future<List<GetWarehouseData>> getWarehouseListArray() async {
    ...
    List<GetWarehouseData> list = res.isNotEmpty
        ? res.map((c) => GetWarehouseData.fromJson(c)).toList()
        : [];
    return list;
  }

  Future<List<GetWarehouseData>> getWarehouseDataByCompany(
      var companyId) async {
    var dbClient = await db;
    // print("SELECT * FROM format where brand_id='$brandId'");
    final res = await dbClient!
        .rawQuery("SELECT * FROM warehouse where company_id='$companyId'");
    List<GetWarehouseData> list = res.isNotEmpty
        ? res.map((c) => GetWarehouseData.fromJson(c)).toList()
        : [];
    return list;
  ...
  Future<int> deleteWarehouse(int warehouseId) async {
    var dbClient = await db;
    return await dbClient!
        .delete('warehouse', where: 'warehouse_id=?', whereArgs: [warehouseId]);
  }

  Future<int> updateWarehouse(WarehouseModel warehouseModel) async {
    var dbClient = await db;
    return await dbClient!.update('warehouse', warehouseModel.toMap(),
        where: 'warehouse_id=?', whereArgs: [warehouseModel.warehouseId]);
  }
  ...
  Future<List<GetDescriptionData>> getDescriptionListRecords() async {
    ...
    List<GetDescriptionData> list = res.isNotEmpty
        ? res.map((c) => GetDescriptionData.fromJson(c)).toList()
        : [];
    return list;
  }
  ...
  Future<int> updateProduct(ProductModel productModel) async {
    var dbClient = await db;
    return await dbClient!.update('product', productModel.toMap(),
        where: 'product_id=?', whereArgs: [productModel.productId]);
  }
  ...
  Future<List<GetCompanyData>> getCompanyListArray() async {
    ...
    List<GetCompanyData> list = res.isNotEmpty
        ? res.map((c) => GetCompanyData.fromJson(c)).toList()
        : [];
    return list;
  }
  ...
  Future<int> deleteCompany(int companyId) async {
    var dbClient = await db;
    return await dbClient!
        .delete('company', where: 'company_id=?', whereArgs: [companyId]);
  }

  Future<int> updateCompany(CompanyModel companyModel) async {
    var dbClient = await db;
    return await dbClient!.update('company', companyModel.toMap(),
        where: 'company_id=?', whereArgs: [companyModel.companyId]);
  }

  Future<List<GetDescriptionData>> getDescriptionListArray(
      var brandId, var formatId, var variantId) async {
    var dbClient = await db;
    final res = await dbClient!.rawQuery(
        "SELECT * FROM product where brand_id='$brandId' and format_id='$formatId' and variant_id='$variantId'");
    List<GetDescriptionData> list = res.isNotEmpty
        ? res.map((c) => GetDescriptionData.fromJson(c)).toList()
        : [];
    return list;
  ...
  Future<List<GetDescriptionData>> getDescriptionRecord(
      var brandId, var formatId, var variantId, var descriptionId) async {
    var dbClient = await db;
    final res = await dbClient!.rawQuery(
        "SELECT * FROM product where brand_id='$brandId' and format_id='$formatId' and variant_id='$variantId' and product_id='$descriptionId'");
    List<GetDescriptionData> list = res.isNotEmpty
        ? res.map((c) => GetDescriptionData.fromJson(c)).toList()
        : [];
    return list;
  ...
  Future<AdminUsersModel> insertAdminUser(
      AdminUsersModel adminUserModel) async {
    var dbClient = await db;
  ...
  Future<List<AdminUsersModel>> getAdminUsersList() async {
    var dbClient = await db;
    final List<Map<String, Object?>> queryResult =
        await dbClient!.query("admin_users");
    return queryResult.map((e) => AdminUsersModel.fromMap(e)).toList();
  }

  Future<int> deleteAdminUser(int adminUserId) async {
    var dbClient = await db;
    return await dbClient!.delete('admin_users',
        where: 'admin_user_id=?', whereArgs: [adminUserId]);
  }

  Future<int> updateAdminUser(AdminUsersModel adminUserModel) async {
    var dbClient = await db;
    return await dbClient!.update('admin_users', adminUserModel.toMap(),
        where: 'admin_user_id=?', whereArgs: [adminUserModel.adminUserId]);
  }
  ...
  Future<dynamic> getAdminUser(String email) async {
    var dbClient = await db;
    return await dbClient!
        .query("admin_users", where: "email = ?", whereArgs: [email], limit: 1);
  }
  ...
  Future<String> getLastSyncDate() async {
    ...
    if (res.length == 0) {
      ...
      dbHelper.insertSyncdate(
          SyncDate(syncId: 1, syncCode: 'lastsyncdate', syncDate: null));
  ...
  Future<int> updateSyncDate(SyncDate syncDate) async {
    var dbClient = await db;
    return await dbClient!.update('sync_date', syncDate.toMap(),
        where: 'sync_id=?', whereArgs: [syncDate.syncId]);
  }
  ...
  Future<void> fetchAllData(lastSyncDate) async {
    ...
    for (var data in response_data) {
      ...
      print(datetime);
      dbHelper.updateSyncDate(
          SyncDate(syncId: 1, syncCode: "lastsyncdate", syncDate: datetime));
    }
  }
  ...
  Future<void> syncDatabase() async {
    ...
    syncDate.then((value) =>
        {lastSyncDate = value, dbHelper!.fetchAllData(lastSyncDate)});
  }
  ...
  Future<void> clearLocalDatabase() async {
    ...
    dbHelper.updateSyncDate(
        SyncDate(syncId: 1, syncCode: "lastsyncdate", syncDate: ""));
  }
}
